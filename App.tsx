...
const parseResult = (text: string, links: GroundingLink[]) => {
  ...
  for (let i = 1; i < dayBlocks.length; i += 2) {
    ...
    // [PLACE] 단위로 나누되, 첫 번째 빈 배열 요소는 무시
    ...
    placeBlocks.forEach(block => {
      ...
      // 이름 추출: 다음 태그([)가 나오기 전까지의 첫 줄
      const name = trimmedBlock.split('\n')[0].replace(/\[.*\]/, '').trim();
      
      // 이름이 없으면 가짜 데이터로 간주하고 스킵 (빈 카드 방지)
...
const App: React.FC = () => {
  ...
  const SectionTitle = ({ num, text }: { num: string, text: string }) => (
    <h3 className="text-xl font-black flex items-center gap-3 mb-4">
      <span className="w-7 h-7 bg-lazy-900 text-white rounded-full flex items-center justify-center text-xs">{num}</span>
      {text}
    </h3>
  );
      ...
      <main className="flex-1 pt-28 pb-10 px-6 max-w-2xl mx-auto w-full">
        {step === 'home' && (
          <div className="fade-in-up space-y-10 py-10 text-center">
            <div className="relative rounded-[3rem] overflow-hidden aspect-video shadow-2xl bg-lazy-900 group">
            ...
            <button onClick={() => setStep('form')} className="w-full py-8 bg-lazy-900 text-white rounded-[2.5rem] font-black text-xl shadow-xl hover:bg-lazy-800 transition-all">
              여정 설계 시작하기
            </button>
          </div>
        )}
        ...
        {step === 'form' && (
          <form onSubmit={handleSubmit} className="fade-in-up space-y-12 pb-20">
            <section>
              ...
              <div className="grid grid-cols-2 gap-3 mb-3">
                {Object.values(Duration).map(d => (
                  <button key={d} type="button" onClick={()=>setRequest({...request, duration:d})} className={`p-4 rounded-3xl border-2 text-sm font-bold transition-all ${request.duration === d ? 'border-lazy-500 bg-lazy-500 text-white shadow-lg shadow-lazy-100' : 'border-slate-100 bg-white text-slate-400'}`}>
                    {d}
                  </button>
                ))}
              </div>
              <div className="grid grid-cols-2 gap-3">
                {Object.values(TransportMode).map(m => (
                  <button key={m} type="button" onClick={()=>setRequest({...request, transport:m})} className={`p-4 rounded-3xl border-2 text-sm font-bold flex items-center justify-center gap-2 ${request.transport === m ? 'border-lazy-900 bg-lazy-900 text-white' : 'border-slate-100 bg-white text-slate-400'}`}>
            ...
            <section>
              ...
              <div className="grid grid-cols-3 gap-3">
                {Object.values(Participant).map(p => (
                  <button key={p} type="button" onClick={()=>setRequest({...request, participants:p})} className={`p-4 rounded-3xl border-2 text-sm font-bold ${request.participants === p ? 'border-lazy-500 bg-lazy-500 text-white shadow-lg shadow-lazy-100' : 'border-slate-100 bg-white text-slate-400'}`}>
                    {p.split(' ')[0]}
                  </button>
                ))}
              </div>
            ...
            <section>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                {Object.values(Region).map(r => (
                  <button key={r} type="button" onClick={()=>setRequest({...request, region:r})} className={`p-3 rounded-2xl border-2 text-xs font-black transition-all ${request.region === r ? 'border-lazy-900 bg-lazy-900 text-white' : 'border-slate-100 bg-white text-slate-400'}`}>
                    {r.split(' ')[0]}
                  </button>
                ))}
              </div>
            ...
            <section>
              ...
              <div className="grid grid-cols-2 gap-3">
                {Object.values(Budget).map(b => (
                  <button key={b} type="button" onClick={()=>setRequest({...request, budget:b})} className={`p-4 rounded-3xl border-2 text-sm font-bold ${request.budget === b ? 'border-lazy-500 bg-lazy-500 text-white shadow-lg shadow-lazy-100' : 'border-slate-100 bg-white text-slate-400'}`}>
                    {b}
                  </button>
                ))}
              </div>
            ...
            <section>
              ...
              <button type="button" onClick={()=>setRequest({...request, includeFood:!request.includeFood})} className={`w-full p-6 rounded-4xl border-2 flex items-center justify-between transition-all ${request.includeFood ? 'border-lazy-500 bg-lazy-50 text-lazy-500' : 'border-slate-100 text-slate-400'}`}>
              ...
              <div className="p-8 bg-slate-50 rounded-5xl border border-slate-100 space-y-8">
                <div className="space-y-4">
                  <div className="flex justify-between font-black text-sm text-slate-600">
                    <span>이동 강도</span>
                    <span className="text-lazy-500">Lv.{request.lazinessLevel}</span>
                  </div>
                ...
                <div className="grid grid-cols-1 gap-2">
                  {Object.values(TravelStyle).map(s => (
                    <button key={s} type="button" onClick={()=>setRequest({...request, style:s})} className={`p-4 rounded-2xl border-2 text-left text-xs font-bold transition-all ${request.style === s ? 'border-lazy-900 bg-lazy-900 text-white' : 'border-white bg-white text-slate-400'}`}>
                      {s}
                    </button>
                  ))}
                </div>
            ...
            <button type="submit" className="w-full py-10 bg-lazy-500 text-white rounded-[3rem] font-black text-2xl shadow-2xl shadow-lazy-200 hover:scale-[1.02] active:scale-[0.98] transition-all">
              최소 동선 계산하기
            </button>
          </form>
        )}
        ...
        {step === 'result' && parsedData && (
          <div className="fade-in-up space-y-12 pb-20">
            <div className="bg-lazy-900 rounded-[3rem] p-8 sm:p-12 text-white space-y-6 shadow-2xl relative overflow-hidden">
              <div className="absolute top-8 right-8 bg-lazy-500 px-4 py-2 rounded-2xl font-black text-xs">LV.{request.lazinessLevel}</div>
            ...
            <div className="space-y-12">
              {parsedData.days.map((day: any) => (
                <div key={day.day} className="space-y-6">
                  <h3 className="text-2xl font-black text-lazy-900 px-2">Day {day.day}</h3>
                      ...
                      <div key={idx} className="lazy-card p-8 space-y-4">
                        <div className="flex justify-between items-start">
                          ...
                          <div className="flex gap-1">
                            {[...Array(5)].map((_, i) => (
                              <span key={i} className={`material-symbols-rounded text-xs ${i < act.photo ? 'text-lazy-500' : 'text-slate-100'}`}>photo_camera</span>
                            ))}
                          </div>
                        </div>
                        ...
                        {act.tip && (
                          <div className="bg-slate-50 p-4 rounded-2xl flex gap-3 items-start">
                            <span className="material-symbols-rounded text-lazy-500 text-sm">tips_and_updates</span>
                            <p className="text-xs font-bold text-slate-700">{act.tip}</p>
                          </div>
                        )}
                        {act.mapLink && (
                          <a href={act.mapLink.uri} target="_blank" className="inline-flex items-center gap-2 text-lazy-500 font-black text-xs">
            ...
            <div className="pt-10 grid grid-cols-2 gap-4">
              <button onClick={savePlan} className="py-6 bg-lazy-500 text-white rounded-3xl font-black text-lg flex items-center justify-center gap-2 shadow-xl shadow-lazy-100 hover:scale-[1.02] transition-all">
                <span className="material-symbols-rounded">bookmark</span> 저장
              </button>
              <button onClick={() => setStep('form')} className="py-6 bg-lazy-900 text-white rounded-3xl font-black text-lg flex items-center justify-center gap-2 hover:scale-[1.02] transition-all">
                  ...
                  <div key={t.id} onClick={() => { setRawResult({ text: t.content, links: t.links }); setStep('result'); window.scrollTo(0, 0); }} className="lazy-card p-6 cursor-pointer group relative overflow-hidden">
                    <div className="flex justify-between items-start pr-10">
                      <div className="space-y-2">
                        ...
                        <h3 className="text-xl font-black group-hover:text-lazy-500 transition-colors pr-4">{t.title}</h3>
